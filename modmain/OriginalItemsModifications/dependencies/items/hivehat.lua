...
local function hivehat()
    ...
    -- 靠近杀人蜂巢不出杀人蜂：为什么不执行啊？...哦，因为 playerprox 写成 playerpro 了 ...
...
items_fns.hivehat.onpreload = function(inst, is_reload_game)
    if not decision_fn(inst, "hivehat") then
        return ;
    end
end

items_fns.hivehat.onload = function(inst, is_reload_game)
    if not decision_fn(inst, "hivehat") then
        return ;
    end
    ...
    if is_reload_game then
        inst.components.armor.absorb_percent = TUNING.ARMORRUINS_ABSORPTION;
        inst.components.armor.maxcondition = TUNING.ARMOR_HIVEHAT;
    else
