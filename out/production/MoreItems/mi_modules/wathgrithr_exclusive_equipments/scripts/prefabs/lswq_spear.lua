local IMAGES_ROOT = "scripts/mi_modules/wathgrithr_exclusive_equipments/images/";
            ...
            for k, v in pairs(ents) do
                if v ~= doer and v:IsValid() and v.components.combat and v.components.health and not v.components.health:IsDead() and doer.components.combat:CanTarget(v) and not doer.components.combat:IsAlly(v) then
                    ...
                    if doer:IsValid() and doer.components.hunger then
                        doer.components.hunger:DoDelta(-3)
                    end
                    break
                end
            end
...
local function fn()
    ...
    inst.components.inventoryitem.atlasname = IMAGES_ROOT .. "inventoryimages/lswq_spear.xml"
    ...
    inst.components.rechargeable:SetRechargeTime(25)
