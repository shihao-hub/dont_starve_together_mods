...
local function MakeHat(prefabname, assets, animstate, overridesymbol)
    ...
    local fn;
    if string.find(prefabname, "_pigcrownhat") then
        fn = pigcrownhat; -- TEMP!!!
    else
        fn = simple;
    end
...
if config_data.xxxhat then
    table.insert(hats, MakeHat("mie_xxxhat", {
        Asset("ANIM", "anim/hat_xxx.zip"),
    }, { "xxxhat", "hat_xxx", "anim" }, { "hat_xxx", "swap_hat" }));
end
