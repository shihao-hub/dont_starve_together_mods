...
local function ShouldGoBig(self)
    ...
    --    print("", v.components.sanity:IsCrazy());
    --    print("", v.prefab == "willow" and v.components.sanity.current < 100)
    --end
    ...
    for i, v in ipairs(AllPlayers) do
        if v:HasTag("bernieowner") and
                ...
                (v.components.sanity:IsCrazy() or v.prefab == "willow" and v.components.sanity.current < 100) and
...
function BernieBrain:OnStart()
    local root = PriorityNode({
        IfNode(
                function()
                    print("IfNode-Go Big")
                    print("", not self.inst.sg:HasStateTag("busy"));
                    print("", not self.inst.components.timer:TimerExists("transform_cd"));
                    print("", ShouldGoBig(self));
                ...
                "Go Big",
                ActionNode(function()
                    DoGoBig(self.inst, self._leader)
                end)),

        --Get the attention of nearby sanity monsters.
        WhileNode(
                function()
                    self._targets = not (self.inst.sg:HasStateTag("busy") or self.inst.components.timer:TimerExists("taunt_cd"))
