...
function fns.RemoveTag(inst, tag)
...
local HANG_UP_ISLAND_TAG = "more_items_hang_up_island";

local function OnTakeEffect(inst)
    local x, y, z = inst.Transform:GetWorldPosition();
    local ents = TheSim:FindEntities(x, 0, z, 16, { "player" }, { "INLIMBO", "playerghost" });

    local cache = {};

    for _, v in ipairs(ents) do
        if v and v:IsValid() and v.components.hunger and v.components.sanity and v.components.health and v.components.temperature and v.components.combat then
            cache[v] = true;
        end
    end

    for v, _ in pairs(CACHE) do
        if v and v:IsValid() and v.components.hunger and not table.containskey(cache, v) then
            --print(tostring(v) .. "移除标签+失效！");
            fns.RemoveTag(v, "notarget");
            v.components.hunger.burnratemodifiers:RemoveModifier(HANG_UP_ISLAND_TAG);
        end
    end
    ...
    for v, _ in pairs(cache) do
        if v and v:IsValid() then
            if not v.components.health:IsDead() then
                --print(tostring(v) .. "添加标签+生效！");
                fns.AddTag(v, "notarget");
                ...
                v.components.temperature:SetTemperature(36.7);
                if v.components.hunger.burnratemodifiers._modifiers[HANG_UP_ISLAND_TAG] == nil then
                    v.components.hunger.burnratemodifiers:SetModifier(HANG_UP_ISLAND_TAG, 0);
                end
            end
        end
    end
...
local function fn()
    ...
    ---------------------------------------------------------------------------
    ---------------------------------------------------------------------------
    inst:DoPeriodicTask(1, OnTakeEffect, 1);
    ---------------------------------------------------------------------------
    ---------------------------------------------------------------------------
    ...
    inst._ripples = SpawnPrefab("mone_watertree_pillar_ripples")
    inst._ripples.entity:SetParent(inst.entity)

    inst.roots = SpawnPrefab("mone_watertree_pillar_roots")
...
return Prefab("mone_watertree_pillar", fn, assets, prefabs),
Prefab("mone_watertree_pillar_ripples", ripples_fn, assets),
Prefab("mone_watertree_pillar_roots", roots_fn, assets)
