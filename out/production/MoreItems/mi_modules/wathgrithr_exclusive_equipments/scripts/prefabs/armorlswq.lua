...
local function huifu(inst)
    ...
    if owner and owner.components.singinginspiration then
        ...
        if songs > 0 and inst.components.armor then
            inst.components.armor:SetCondition(inst.components.armor.condition + 2 * songs)
        end
    end
end
...
local function fn()
    ...
    inst.components.inventoryitem.atlasname = IMAGES_ROOT .. "inventoryimages/armorlswq.xml"
