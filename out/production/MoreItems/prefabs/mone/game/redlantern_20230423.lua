...
function fns.ontakefuel(inst)
    if inst.components.equippable:IsEquipped() then
        turnon(inst)
    end
end

function fns.ondeploy(inst, pt, deployer)
    local structure = SpawnPrefab("mone_redlantern_structure");
    structure.Transform:SetPosition(pt:Get())

    structure.SoundEmitter:PlaySound("dontstarve/common/chest_craft")
...
local function fn()
    ...
    inst.components.fueled:SetUpdateFn(fns.fuelupdate)
    inst.components.fueled:SetTakeFuelFn(fns.ontakefuel)
    inst.components.fueled:SetFirstPeriod(TUNING.TURNON_FUELED_CONSUMPTION, TUNING.TURNON_FULL_FUELED_CONSUMPTION)
    inst.components.fueled.accepting = true

    inst:AddComponent("deployable")
    inst.components.deployable.ondeploy = fns.ondeploy;
...
return Prefab("mone_redlantern", fn, assets),
Prefab("mone_redlantern_structure", fn_structure, assets),
MakePlacer("mone_redlantern_placer", "redlantern", "redlantern", "idle_loop"),
